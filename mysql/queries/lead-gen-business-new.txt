...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`lead_gen_business` /*!40100 DEFAULT CHARACTER SET utf8 */;
...
SELECT COUNT(leads.leads_id) Num_Leads
FROM
leads JOIN sites
ON sites.site_id = leads.site_id
WHERE 
	YEAR(leads.registered_datetime) > 2010
	AND
    leads.registered_datetime < "2011-02-16";
    
SELECT clients.first_name,
	   clients.last_name,
       SUM(leads.leads_id) AS Num_Leads
FROM 
clients JOIN sites
...
WHERE 
	...
	AND
    YEAR(leads.registered_datetime) = 2011
GROUP BY clients.first_name, 
         clients.last_name
